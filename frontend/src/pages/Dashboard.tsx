...
interface YearlySummary {
  year: number; realized_profit_krw: number; sells: number; wins: number
  win_rate: number; avg_hold_days: number; year_principal: number
  year_delta: number; return_rate: number
}
    ...
    <div className="space-y-6">
      ...
      {yearly_summary.length > 0 && (
        <>
          ...
          {selectedYear && (() => {
            ...
            return (
              <div className="grid grid-cols-4 gap-4">
                <KpiCard label="실현손익" value={fmtKrw(y.realized_profit_krw)} valueClass={colorClass(y.realized_profit_krw)} />
                ...
                <KpiCard label="수익률" value={fmtRate(y.return_rate)} valueClass={colorClass(y.return_rate)}
                  sub={`매도 ${y.sells}회  승률 ${y.win_rate}%  평균 ${y.avg_hold_days}일`} />
              </div>
            )
          })()}
        </>
      ...
      {sections.map(section => (
        <div key={section.name}>
          ...
          <SectionHeader
            title={section.name}
            sub={`총 ${fmtAmount(section.total, section.currency)}`}
          />
          <div className="grid grid-cols-2 gap-4">
            {/* 월별 수익 막대 */}
            <Card className="p-5">
              <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-4">월별 수익</p>
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={section.monthly} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
                  <XAxis dataKey="month" tick={{ fontSize: 11 }} tickLine={false} axisLine={false} />
                  <YAxis tick={{ fontSize: 11 }} tickLine={false} axisLine={false}
                    tickFormatter={v => section.currency === 'USD' ? `$${(v/1000).toFixed(0)}K` : `${(v/10000).toFixed(0)}만`} />
            ...
            {/* 종목별 누적 수익 */}
            <Card className="p-5">
              <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-4">종목별 누적 수익</p>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {MONTHS.map((month, idx) => {
                    const row = section.monthly[idx]
                    return (
                      <tr key={month} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                        <td className="px-4 py-2.5 text-slate-600 font-medium">{month}</td>
                        {section.stocks.map(s => {
                          const stockRow = (section.stocks_monthly.find(r => r.month === month) || {}) as Record<string, number | string>
